...
export default function EditSet(){
    ...
    // props to carry over prevPage info and setId of the set to edit
    ...
    // grab set with id setId from api once the prop has been read
    useEffect(() => {
        if(setId){
    ...
    // update variables to what they are currently set to from the api call once setObj is allocated
    useEffect(() => {
        if(setObj.name != null){
            setSetName(setObj.name);
            setSetTheme(setObj.theme);
            setSetNum(setObj.set_num);
            setSetPieces(setObj.piece_num);
        }
    }, [setObj]);

    // put request to api to update changes made to a set for the logged in user
    const handleSubmit = async (e) => {
        // prevent default form submission
        e.preventDefault();

        try {
            const response = await axios.put(`http://localhost:8000/api/set/${setId}/`, {
                name: setName,
                theme: setTheme,
                set_num: setNum,
                piece_num: setPieces
        ...
        <div className="outer-container outer-container-add">
            <div className="add-top-container">
                <div className="add-title-wrapper">
                    <h1 className="add-title">Edit Your Set</h1>
                </div>
                <div className="add-subtitle-wrapper">
                    <p className="add-subtitle">You can change any information about this set!</p>
                </div>
            </div>
            <div className="add-middle-container">
                <form className="add-form" onSubmit={handleSubmit}>
                    ...
                    <div className="add-field">
                        ...
                        { setNumError && (<h2>{setNumError}</h2>) }
                    </div>
                    <div className="add-field">
                        <label className="add-field-title">Number of Pieces:</label>
                        ...
                        { setPiecesError && (<h2>{setPiecesError}</h2>) }
                    </div>
                    <div className="add-buttons">
                        <button className="add-cancel-button" type="button" onClick={handleCancel}>Cancel</button>
                        <button className="add-submit-button" type="submit" disabled={setName.length == 0 || setTheme.length == 0 || setNum.length == 0 || setPiecesError.length > 0 || setNumError.length > 0}>Save</button>
                    </div>
                </form>
            </div>
        </div>
